...
impl Compiler {
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<(), CompileError> {
        match expr {
            Expr::Ident(ident, _, span) => return self.compile_ident(ident, span),
            Expr::Literal(l, _) => return self.compile_literal(l),
            Expr::Prefix(op, expr, _) => return self.compile_prefix(op, expr),
            Expr::Infix(op, lhs, rhs, _) => return self.compile_infix(op, lhs, rhs),
            _ => {}
        }
